...
async def get_tasks_repo(db_session: AsyncSession = Depends(get_db_session)) -> TaskRepository:

    return TaskRepository(db_session)


async def get_tasks_cacherepository() -> TaskCache:
    redis_connection = get_redis_connection()
    return TaskCache(redis_connection)
    

async def get_tasks_service(task_repository: TaskRepository = Depends(get_tasks_repo), 
